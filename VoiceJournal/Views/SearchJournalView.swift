...
private enum FutureLetterCollection: String, Identifiable {
    case draft
    case scheduled
    case delivered
    ...
    var title: String {
        switch self {
        case .draft: "Draft Letters"
        case .scheduled: "Scheduled Letters"
        case .delivered: "Delivered Letters"
        }
    }

    var detailTitle: String {
        switch self {
        case .draft: "Draft Letter"
        case .scheduled: "Scheduled Letter"
        case .delivered: "Delivered Letter"
        }
    }
    ...
    func displayDate(for letter: FutureLetter) -> Date {
        self == .draft ? letter.updatedAt : letter.deliveryDate
    }
}
...
private struct FutureLetterComposerView: View {
    ...
    private var letterCollectionsSection: some View {
        VStack(spacing: 10) {
            ForEach([FutureLetterCollection.draft, .scheduled, .delivered]) { collection in
                Button {
                    deactivateTextEditing()
                    ...
                    HStack(spacing: 12) {
                        ...
                        Text("\(letterCount(for: collection))")
                            .font(selectedFontDesignPreference.font(.callout, weight: .semibold))
                            .foregroundStyle(.secondary)
    ...
    private func letterCount(for collection: FutureLetterCollection) -> Int {
...
private struct FutureLetterDeliveryDatePicker: View {
    ...
    @State private var activePicker: DeliveryPickerComponent?
    ...
    var body: some View {
        ...
        .sheet(item: $activePicker) { component in
            NavigationStack {
                Group {
                    if component == .date {
                        DatePicker(
    ...
    private func selectionButton(title: String, component: DeliveryPickerComponent) -> some View {
        Button {
    ...
    private enum DeliveryPickerComponent: String, Identifiable {
        case date
        case time
        ...
        var accessibilityLabel: String {
            switch self {
            case .date: "Delivery date"
            case .time: "Delivery time"
            }
        }
    }
}
...
private struct FutureLetterListView: View {
    @Environment(\.modelContext) private var modelContext
    @AppStorage("journalFontDesignPreference") private var journalFontDesignPreference = JournalFontDesignPreference.system.rawValue
    ...
    private var displayedLetters: [FutureLetter] {
        let matchingLetters = letters.filter { collection.includes($0, now: currentDate) }
        switch collection {
        case .draft:
            return matchingLetters.sorted { $0.updatedAt > $1.updatedAt }
        case .scheduled:
            return matchingLetters.sorted { $0.deliveryDate < $1.deliveryDate }
        case .delivered:
            return matchingLetters.sorted { $0.deliveryDate > $1.deliveryDate }
        }
    }
    ...
    private func letterTitle(_ letter: FutureLetter) -> String {
        ...
        return trimmedTitle.isEmpty ? "Untitled Letter" : trimmedTitle
    }
...
private struct FutureLetterDestinationView: View {
    ...
    var body: some View {
        if let letter = letters.first(where: { $0.id == letterID }) {
            if letter.notificationIdentifier == nil {
                FutureLetterDetailView(letter: letter)
            } else {
                FutureLetterReadOnlyView(
                    letter: letter,
                    collection: letter.deliveryDate > Date() ? .scheduled : .delivered
                )
            }
        } else {
            ContentUnavailableView(
...
private struct FutureLetterReadOnlyView: View {
    @AppStorage("journalFontDesignPreference") private var journalFontDesignPreference = JournalFontDesignPreference.system.rawValue
    let letter: FutureLetter
    let collection: FutureLetterCollection
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                if collection == .delivered {
                    deliveredTimeSection
                }
            }
            .padding()
        }
        .background(AppThemeBackground())
        .navigationTitle(collection.detailTitle)
        .navigationBarTitleDisplayMode(.inline)
    }

    private var deliveredTimeSection: some View {
        HStack(spacing: 12) {
...
private struct ThemeCloudView: View {
    ...
    private var mainTheme: (theme: String, count: Int)? {
        displayThemes.max { first, second in
            first.count == second.count ? first.theme > second.theme : first.count < second.count
        }
    }

    private var arrangedThemes: [(theme: String, count: Int)] {
        guard let mainTheme else { return [] }
        var skippedMain = false
        let surrounding = displayThemes.filter { item in
            if !skippedMain, item.theme == mainTheme.theme, item.count == mainTheme.count {
                skippedMain = true
                return false
            }
            return true
        ...
        .sorted { first, second in
            ...
            return stableKey(for: first.theme) < stableKey(for: second.theme)
        }
        return [mainTheme] + surrounding
    }
        ...
        ThemeCloudOrbitLayout(spacing: 5) {
            ForEach(Array(arrangedThemes.enumerated()), id: \.element.theme) { index, item in
                ...
                Text(item.theme)
                    ...
                    .foregroundStyle(themeColor(at: index))
                    .lineLimit(1)
    ...
    private func themeColor(at index: Int) -> Color {
        let colors: [Color] = [
            .accentColor,
            Color(hex: "#2A9D8F"), Color(hex: "#E76F51"), Color(hex: "#5E60CE"),
            Color(hex: "#D18400"), Color(hex: "#0077B6"), Color(hex: "#C44569"),
            Color(hex: "#4D908E"), Color(hex: "#9B5DE5"), Color(hex: "#F15BB5")
        ]
...
private struct ThemeCloudOrbitLayout: Layout {
    ...
    func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout ()) {
        ...
        for index in subviews.indices.dropFirst() {
            ...
            occupied.append(rect(centeredAt: center, size: collisionSize))
        }
    }
    ...
    private func searchCenters(in bounds: CGRect, preferred: CGPoint, size: CGSize) -> [CGPoint] {
        ...
        guard minX <= maxX, minY <= maxY else { return [CGPoint(x: bounds.midX, y: bounds.midY)] }

        var centers: [CGPoint] = []
        for y in stride(from: minY, through: maxY, by: 5) {
            for x in stride(from: minX, through: maxX, by: 5) {
    ...
    private func clampedCenter(_ center: CGPoint, size: CGSize, in bounds: CGRect) -> CGPoint {
        CGPoint(
            x: min(max(center.x, bounds.minX + size.width / 2), bounds.maxX - size.width / 2),
            y: min(max(center.y, bounds.minY + size.height / 2), bounds.maxY - size.height / 2)
        )
    }
    ...
    private func firstAvailableCenter(
        from centers: [CGPoint],
        collisionSize: CGSize,
        occupied: [CGRect]
    ) -> CGPoint? {
        let gaps: [CGFloat] = [spacing, max(CGFloat(2), spacing - 3), 0]
        for gap in gaps {
            if let center = centers.first(where: { candidate in
    ...
    private func leastOverlappingCenter(
        from centers: [CGPoint],
        collisionSize: CGSize,
        occupied: [CGRect]
    ) -> CGPoint {
        centers.min { first, second in
            overlapArea(of: rect(centeredAt: first, size: collisionSize), with: occupied) <
                overlapArea(of: rect(centeredAt: second, size: collisionSize), with: occupied)
        } ?? .zero
    }

    private func overlapArea(of rect: CGRect, with occupied: [CGRect]) -> CGFloat {
        occupied.reduce(0) { result, other in
